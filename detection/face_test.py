...
name='/home/lee/bigdata/wiki_new.pkl'
def load_data(name):
    with open(name,'rb') as f:
        return pickle.load(f)

gray=load_data(name)
valid=gray['valid_dataset']
#print valid
gray1=np.array(np.reshape(valid[56,:,:,:],(128,128)),dtype='uint8')
print gray1
cv2.imshow('gray image',gray1)
cv2.waitKey(10000)
i=1
# 探测图片中的人脸
faces = face_cascade.detectMultiScale(gray1,scaleFactor=1.1,minNeighbors=5,minSize=(5,5),flags = cv2.cv.CV_HAAR_SCALE_IMAGE)

print ("found {0} faces!".format(len(faces)))

image1=np.copy(gray1)
